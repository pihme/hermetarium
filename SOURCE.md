# gh-pages source

This branch holds the static GitHub Pages site for `pihme/hermetarium`.

Generated on 2026-09-26 from `main` at commit `97a5de859ac438c814f320da9aefa1972ede00c0`.
Self-contained `index.html` and `chronik/index.html` (timeline; inline CSS, no build step, no trackers) plus `.nojekyll`.
Generator: `gen.py` (layout B, sidebar handbook) + per-site content script + `chronik-<repo>.json` timeline data.
Regenerate when `main` changes; do not merge this branch into `main`.

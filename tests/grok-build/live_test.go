//go:build live

package grokbuild

import (
	"os"
	"strings"
	"testing"

	"github.com/pihme/hermetarium/supervisor"
)

func TestGrokBuildLive(t *testing.T) {
	if os.Getenv("HERMETARIUM_XAI_API_KEY") == "" {
		t.Skip("HERMETARIUM_XAI_API_KEY unset")
	}
	root, err := supervisor.Root()
	if err != nil {
		t.Fatal(err)
	}
	id, err := supervisor.NewID()
	if err != nil {
		t.Fatal(err)
	}
	inst, err := supervisor.CreateWeakExample(root, id, supervisor.ExampleGrok)
	if err != nil {
		t.Fatal(err)
	}
	defer supervisor.DestroyWeak(id)
	got, err := supervisor.Call(inst.InboundURL, "Run the shell command id -u and reply with only the number.")
	if err != nil {
		t.Fatal(err)
	}
	if !strings.Contains(got, "0") {
		t.Fatalf("live reply %q", got)
	}
}
